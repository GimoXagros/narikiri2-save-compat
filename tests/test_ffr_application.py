...
from apply_ffr_v09 import checked, NAME, SOURCE, TARGET
from bps import create_bps

class FfrApplicationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.target = (ROOT/'output/v0.9-final-build'/(NAME+'.gba')).read_bytes()
    ...
    def test_existing_output_preserved_before_reading_inputs(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            r = subprocess.run([sys.executable, str(ROOT/'tools/apply_ffr_v09.py'), str(Path(d)/'absent.gba'), '--output', str(output)], capture_output=True)
